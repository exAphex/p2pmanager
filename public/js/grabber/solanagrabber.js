...
const apiStake = 'https://public-api.solanabeach.io/v1/account/';
...
const getSolana = async (address) => {
  ...
  const responseStake = await fetch(apiStake + address + "/stakes?limit=10&offset=0", {method: 'GET', headers: {'Content-Type': 'application/json', 'Accept': 'application/json', 'User-Agent': 'PostmanRuntime/7.26.8'}});
  ...
  for (const acc in stakeAccounts) {
    if (stakeAccounts.hasOwnProperty(acc)) {
      ...
      total += obj.lamports * 1;
      stake += obj.lamports * 1;
    }
  }
